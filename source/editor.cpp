...
public:
    ...
    void draw(VSTGUI::CDrawContext* ctx) override
    {
        ...
        ctx->drawString(editor_->getZoomFactor()>.84 ? "UI 100%" : "UI 68%", r, VSTGUI::kCenterText);
        setDirty(false);
    }
    ...
    VSTGUI::CMouseEventResult onMouseDown(VSTGUI::CPoint&, const VSTGUI::CButtonState&) override
    {
        ...
        editor_->setUserZoom(editor_->getZoomFactor()>.84 ? .68 : 1.0);
        invalid();
        return VSTGUI::kMouseEventHandled;
    }
...
: VST3Editor(c, "view", "SMX3.uidesc"), controller_(c)
{
    setZoomFactor(.68);
