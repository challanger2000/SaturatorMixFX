...
tresult PLUGIN_API Processor::process(ProcessData& d)
{
    if (mixFxEngaged_)
    {
        ...
        {
            auto& inBus = d.inputs[b];
            auto& outBus = d.outputs[b];
            const int32 channels = std::min(inBus.numChannels, outBus.numChannels);
            if (d.symbolicSampleSize == kSample64)
            {
                for (int32 ch = 0; ch < channels; ++ch)
                {
                    auto* src = inBus.channelBuffers64 ? inBus.channelBuffers64[ch] : nullptr;
                    auto* dst = outBus.channelBuffers64 ? outBus.channelBuffers64[ch] : nullptr;
                    if (!src || !dst || src == dst)
                        continue;
            ...
            else if (d.symbolicSampleSize == kSample32)
            {
                for (int32 ch = 0; ch < channels; ++ch)
                {
                    auto* src = inBus.channelBuffers32 ? inBus.channelBuffers32[ch] : nullptr;
                    auto* dst = outBus.channelBuffers32 ? outBus.channelBuffers32[ch] : nullptr;
                    if (!src || !dst || src == dst)
                        continue;
                    for (int32 n = 0; n < d.numSamples; ++n)
                        dst[n] = src[n];
