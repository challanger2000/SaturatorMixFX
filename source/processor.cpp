...
void Processor::resetDsp(){for(auto& s:channelState_)s={};smoothDrive_=drive_;smoothCharacter_=character_;smoothMix_=mix_;smoothOutput_=output_;}
void Processor::updateSmoothers(){double a=1.0-smoothCoeff_;smoothDrive_=smoothCoeff_*smoothDrive_+a*drive_;smoothCharacter_=smoothCoeff_*smoothCharacter_+a*character_;smoothMix_=smoothCoeff_*smoothMix_+a*mix_;smoothOutput_=smoothCoeff_*smoothOutput_+a*output_;}
...
tresult PLUGIN_API Processor::process(ProcessData& d){
    ...
    if(d.inputParameterChanges){for(int32 i=0;i<d.inputParameterChanges->getParameterCount()&&cursorCount<(int)cursors.size();++i){auto*q=d.inputParameterChanges->getParameterData(i);if(!q||q->getPointCount()<=0)continue;ParamID id=q->getParameterId();if(id!=kParamOnOff&&id!=kParamDrive&&id!=kParamCharacter&&id!=kParamMix&&id!=kParamOutput)continue;cursors[(size_t)cursorCount++]={q,0,q->getPointCount(),id};}}
    auto applyAutomation=[&](int32 sample){for(int i=0;i<cursorCount;++i){auto&c=cursors[(size_t)i];while(c.next<c.count){int32 off=0;ParamValue v=0;if(c.q->getPoint(c.next,off,v)!=kResultTrue){++c.next;continue;}if(off>sample)break;v=clamp01(v);switch(c.id){case kParamOnOff:onOff_=v;break;case kParamDrive:drive_=v;break;case kParamCharacter:character_=v;break;case kParamMix:mix_=v;break;case kParamOutput:output_=v;break;default:break;}++c.next;}}};
    bool allBypassed=true;
    auto run=[&](auto**srcs,auto**dsts){using Sample=std::remove_pointer_t<std::remove_pointer_t<decltype(srcs)>>;for(int32 n=0;n<d.numSamples;++n){applyAutomation(n);updateSmoothers();const bool bypass=onOff_>=.5;allBypassed=allBypassed&&bypass;const double pos=clamp01(smoothCharacter_)*2.0;const double wT=std::max(0.0,1.0-pos);const double wI=std::max(0.0,pos-1.0);const double wP=1.0-wT-wI;const double effectiveDrive=shapeDrive(smoothDrive_);const double driveDb=24.0*effectiveDrive,inputGain=dbToGain(driveDb),wet=smoothMix_,dry=1.0-wet,outGain=dbToGain(-18.0+24.0*smoothOutput_);const double trim=(-9.50*wT-19.00*wP-14.47*wI)*effectiveDrive;const double baseComp=(-.46*wT-.52*wP-.40*wI)*driveDb;const double referenceAmount=std::min(1.0,effectiveDrive/.30);const double polishTrimDb=(.73*wT+2.67*wP-.06*wI)*referenceAmount;const double comp=dbToGain(trim+baseComp+polishTrimDb);const double protect=.18*wT+.42*wP+.30*wI;const double attackAmount=.08*wT+.22*wP+.15*wI;for(int32 ch=0;ch<chans;++ch){auto*src=srcs[ch];auto*dst=dsts[ch];if(!src||!dst)continue;double x=(double)src[n];auto&s=channelState_[(size_t)ch];s.lowBand=lowCoeff_*s.lowBand+(1.0-lowCoeff_)*x;s.highSmooth=highCoeff_*s.highSmooth+(1.0-highCoeff_)*x;double low=s.lowBand,high=x-s.highSmooth,mid=x-low-high;const double triCol=.94*low+1.09*mid+.84*high;const double penCol=.84*low+1.10*mid+1.07*high;const double ironCol=1.13*low+1.025*mid+.80*high;const double coloured=wT*triCol+wP*penCol+wI*ironCol;double a=std::abs(x);s.envFast=envFastCoeff_*s.envFast+(1.0-envFastCoeff_)*a;s.envSlow=envSlowCoeff_*s.envSlow+(1.0-envSlowCoeff_)*a;double transient=std::max(0.0,s.envFast-s.envSlow),normTransient=clamp01(transient/(.06+s.envSlow));double dynamicGain=inputGain*(1.0-protect*normTransient);double processedOs=0.0,cleanOs=0.0;for(int os=0;os<kOversample;++os){const double stuffed=(os==0)?(coloured*static_cast<double>(kOversample)):0.0;const double cleanStuffed=(os==0)?(x*static_cast<double>(kOversample)):0.0;const double up=runOversamplingFilter(stuffed,s.osUp);const double cleanUp=runOversamplingFilter(cleanStuffed,s.cleanUp);const double nlT=shapeTriode(up*dynamicGain,s);const double nlP=shapePentode(up*dynamicGain,s);const double nlI=shapeIron(up*dynamicGain,s);const double nl=wT*nlT+wP*nlP+wI*nlI;const double filtered=runOversamplingFilter(nl,s.osDown);const double cleanFiltered=runOversamplingFilter(cleanUp,s.cleanDown);if(os==kOversample-1){processedOs=filtered;cleanOs=cleanFiltered;}}double processed=processedOs*comp;double attackBlend=normTransient*attackAmount;processed=processed*(1.0-attackBlend)+cleanOs*attackBlend;processed=peakProtect(processed);processed=dcBlock(processed,s);const double mixed=(wet<=1.0e-6)?x:(dry*cleanOs+wet*processed);const double active=x+referenceAmount*(mixed-x);dst[n]=(Sample)(bypass?x:(active*outGain));}}};
    if(d.symbolicSampleSize==kSample64)run(in.channelBuffers64,out.channelBuffers64);else if(d.symbolicSampleSize==kSample32)run(in.channelBuffers32,out.channelBuffers32);else return kResultFalse;
    out.silenceFlags=allBypassed?in.silenceFlags:0;return kResultOk;
}
tresult PLUGIN_API Processor::setState(IBStream* s){if(!s)return kResultFalse;IBStreamer f(s,kLittleEndian);double b=0,dr=.30,c=0,m=1,o=.75;if(!f.readDouble(b)||!f.readDouble(dr)||!f.readDouble(c)||!f.readDouble(m)||!f.readDouble(o))return kResultFalse;onOff_=clamp01(b);drive_=clamp01(dr);character_=clamp01(c);mix_=clamp01(m);output_=clamp01(o);smoothDrive_=drive_;smoothCharacter_=character_;smoothMix_=mix_;smoothOutput_=output_;return kResultOk;}
